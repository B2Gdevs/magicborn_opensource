// components/content-editor/ActForm.tsx
// Form for creating/editing acts with React Hook Form
...
type ActFormDataInput = z.infer<typeof actSchema>;

// Wrapper for checkIdUniqueness with EntryType.Act
async function checkActIdUniqueness(
  id: string,
  projectId?: string,
  excludeId?: number
): Promise<{ isUnique: boolean; error?: string }> {
  return checkIdUniqueness(EntryType.Act, id, projectId, excludeId);
}
...
export function ActForm({
  ...
  const { idValidation, validatingId } = useIdValidation({
    ...
    projectId,
    editEntryId,
    checkIdUniqueness: checkActIdUniqueness,
    setError,
    clearErrors,
  });
  ...
  const onSubmitForm = async (data: ActFormDataInput) => {
      ...
      <div className="flex-1 overflow-y-auto min-h-0">
        <form onSubmit={handleSubmit(onSubmitForm)} className="space-y-6 p-6">
          ...
          <BasicInfoSection
            ...
            nameValue={name}
            namePlaceholder="e.g., Act I"
            descriptionValue={watch("description") || ""}
            descriptionPlaceholder="Act description..."
