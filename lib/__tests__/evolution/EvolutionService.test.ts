 ...
 * for Ember Ray:
 *
 * - Runes: exactly the Ember Ray required runes ("F", "A", "R")
 * - Level: at least the blueprint's min level
...
describe("EvolutionService – Ember Ray blueprint", () => {
  ...
  it("Ember Ray: candidate with wrong runes is NOT eligible", () => {
    ...
    const candidate = makeEmberRayCandidate();
    candidate.runes = ["F", "A", "X"]; // missing "R" required by blueprint
  ...
  it("Ember Ray: evolveSpell stamps name, evolvedFrom", () => {
    const candidate = makeEmberRayCandidate();
    ...
    expect(evolved.evolvedFrom).toBe(candidate.id); // ✅ not a hardcoded string
  });
});
